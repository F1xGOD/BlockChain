...
async def interactive(ws, genesis):
    ...
    while True:
        ...
        if cmd=="language" and len(parts)==2:
            ...
            if lg in LANGUAGES:
                preferred_lang=lg
                print(f"🌐 Preferred seed language set to {lg.capitalize()}. "
                      "It takes effect on next wallet creation.")
            else: print("❓ Supported:",", ".join(LANGUAGES)); continue
        if cmd=="security":
            for k,v in (("Seed Phrase",wallet['seed']),
        ...
        if cmd=="send" and len(parts)==3:
            ...
            print("❓ Unknown or bad syntax."); continue
...
async def main():
    ...
    if MY_ADDR == GENESIS:
        print(f"👑 Welcome, Itsuki! 💼 Address: {MY_ADDR}   |   🔥 You ARE the GENESIS.")
    else:
